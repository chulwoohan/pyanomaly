...
Define a new class by inheriting ``CRSPM`` and add a new function ``c_excess_ret_change()`` that defines
...
Below are some examples of setting transaction costs.
...
In the panel data, the future return at t is the return between t and t+1, whereas in the position data, dates are shifted
...
This downloads data every `interval` years along `date_col`. This is a memory efficient way of downloading a large dataset.
